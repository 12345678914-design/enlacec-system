...
export const Header: React.FC<HeaderProps> = ({ onToggleMobileSidebar, activeTabLabel }) => {
  const { currentUser, logout, theme, updateTheme, students, teachers, resources } = useApp();
  ...
  // Mock school alerts state for interaction
  const [notifications, setNotifications] = useState([
    { id: 1, title: 'Inscripción aprobada', desc: 'Estudiante Sofía Valentino registrada con éxito', time: 'Hace 5m', read: false, type: 'success' },
    { id: 2, title: 'Finanzas del mes', desc: 'Reporte de egresos del profesorado listo para revisión', time: 'Hace 2h', read: true, type: 'finance' },
    { id: 3, title: 'Recordatorio Académico', desc: 'Entrega de portafolio para fin de mes', time: 'Ayer', read: true, type: 'academic' }
  ]);
  ...
  return (
    <>
      <header className="sticky top-0 z-35 bg-white/30 dark:bg-slate-900/30 backdrop-blur-xl border-b border-gray-200/50 dark:border-white/10 transition-colors duration-300 h-16 px-4 flex items-center justify-between">
                            ...
                            {results.students.map(s => (
                              ...
                              >
                                <div className="flex items-center gap-2 min-w-0">
                                  <img src={s.avatarUrl} alt="" className="w-5 h-5 rounded-md object-cover" />
                        ...
                        <div className="p-1.5">
                          ...
                          <div className="space-y-0.5 mt-1">
                            {results.teachers.map(t => (
                              ...
                              >
                                <div className="flex items-center gap-2 min-w-0">
                                  <img src={t.avatarUrl} alt="" className="w-5 h-5 rounded-md object-cover" />
                                  <div className="truncate">
        ...
        <div className="hidden md:flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-zinc-400 bg-white/45 dark:bg-white/5 px-3 py-1.5 rounded-xl border border-gray-200/40 dark:border-white/10 backdrop-blur-md">
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2 mb-1">
                              ...
                              <span className="text-[9px] text-gray-400 dark:text-zinc-500 font-mono shrink-0">{n.time}</span>
                            </div>
                            <p className="text-xs text-gray-500 dark:text-zinc-400 leading-normal pr-4">{n.desc}</p>
                          </div>
        ...
        <div className="relative">
          ...
          >
            <img
              src={currentUser?.avatarUrl || 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=120'}
              alt={currentUser?.name}
              referrerPolicy="no-referrer"
              className="w-7.5 h-7.5 rounded-lg object-cover ring-2 ring-white/60 dark:ring-white/10"
            />
              ...
              >
                ...
                <div className="p-1">
                  <button
                    type="button"
                    onClick={logout}
                    className="w-full text-left px-3 py-2.5 hover:bg-red-500/10 text-red-600 dark:text-red-400 flex items-center gap-2 rounded-xl font-bold uppercase tracking-wider text-[10px] transition-colors"
                  >
