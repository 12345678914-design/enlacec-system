...
export const AISystemReview: React.FC = () => {
  ...
  // Run the AI audit via the backend endpoint /api/chat
  const runDiagnostic = async () => {
    ...
    const steps = [
      ...
      'Invocando EnlaceC IA de Google Gemini para auditoría...'
    ];
    ...
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: systemOverviewPrompt,
          systemContext: 'Eres Tesla, el asistente inteligente de ENLACEC. Tu tarea es actuar como un auditor y analista escolar consultivo super inteligente. Tu tono es motivador, analítico, objetivo y formal.'
        })
      });

      if (!response.ok) {
        throw new Error('No se pudo establecer comunicación con el motor de IA de Gemini.');
      }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="space-y-0.5 text-left">
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] font-bold text-indigo-505 dark:text-indigo-400 uppercase tracking-widest bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full">EnlaceC Auditor IA</span>
        ...
        <div className="p-5 border border-dashed border-gray-200 dark:border-zinc-800 rounded-2xl bg-slate-50/50 dark:bg-zinc-950/20 text-center space-y-4 max-w-xl mx-auto">
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-gray-450 leading-relaxed px-4">
              El motor de IA de EnlaceC revisará el registro completo en tiempo real, cotejará ausencias críticas y creará un plan inmediato para optimizar las operaciones de tu plantel.
            </p>
          </div>
          ...
          <div className="flex justify-center pt-1.5">
            ...
            >
              ...
              <span>Ejecutar Diagnóstico EnlaceC IA</span>
            </button>
          </div>
        </div>
      ) : (
                ...
                <div className="md:col-span-7 flex flex-col justify-between space-y-4">
                  <div className="bg-gradient-to-tr from-indigo-50 bg-indigo-50/5 dark:bg-zinc-900/30 border border-indigo-250/30 dark:border-indigo-950/40 rounded-2xl p-5 shrink-0">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-750 dark:text-indigo-400 font-sans">Sugerencias Estratégicas de EnlaceC IA</span>
                    </div>
