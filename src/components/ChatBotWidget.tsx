...
export const ChatBotWidget: React.FC = () => {
  ...
  useEffect(() => {
    const savedKey = localStorage.getItem("edu_gemini_api_key_override") || "";
  ...
  const handleSaveApiKey = (key: string) => {
    ...
    if (cleaned) {
      localStorage.setItem("edu_gemini_api_key_override", cleaned);
    } else {
      localStorage.removeItem("edu_gemini_api_key_override");
    }
  };
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      // Gather current system's context state
      const systemContext = {
        students,
        teachers,
        balance,
      ...
      if (!response.ok) {
        throw new Error(data.error || "Ocurrió un error al consultar con el asistente.");
      }
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 md:right-6 z-[9999] w-[calc(100vw-32px)] sm:w-[420px] h-[550px] md:h-[600px] border border-gray-200/80 dark:border-zinc-800/60 bg-white/95 dark:bg-zinc-900/98 backdrop-blur-xl rounded-[32px] overflow-hidden flex flex-col shadow-2xl"
          >
            ...
            <div className={`p-4 border-b border-gray-150 dark:border-zinc-850 flex items-center justify-between select-none shrink-0 relative bg-gradient-to-r ${cl.gradient} text-white`}>
              ...
              <div className="flex items-center gap-3 relative z-10 text-left">
                <div className="w-10 h-10 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center shadow-inner shrink-0 leading-none">
                  <Bot className="w-5.5 h-5.5 text-white animate-pulse" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <h3 className="font-extrabold text-sm tracking-wide uppercase">EnlaceC AI</h3>
                    <span className="px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest bg-emerald-500 border border-emerald-400 text-white animate-pulse">
                      Gemini 3.5
                    </span>
                  </div>
                  <p className="text-[10px] text-white/80 font-medium">Asistente de Gestión e Innovación Escolar</p>
                </div>
              </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-[10px] font-black text-gray-500 dark:text-zinc-400 uppercase tracking-widest flex items-center gap-1.5">
                        <Key className="w-3 h-3 text-indigo-550" />
                        API Key de Gemini
                      </label>
                      {customKey && (
                        <span className="text-[9px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/20 px-1.5 py-0.5 rounded-md border border-indigo-100 dark:border-indigo-900/30">
                          Personalizada Activa
                        </span>
                      )}
                    </div>
                    
                    <p className="text-[10.5px] leading-relaxed text-gray-505 dark:text-zinc-550 font-medium">
                      Por defecto, el chatbot funciona con la API Key global del servidor (no necesitas hacer nada). Si prefieres usar una clave propia, incorpórala abajo:
                    </p>

                    <div className="relative mt-1">
                      <input
                        id="input-chatbot-custom-api-key"
                        type={showKeyText ? "text" : "password"}
                        value={customKey}
                        onChange={(e) => handleSaveApiKey(e.target.value)}
                        placeholder="Ingresa tu GEMINI_API_KEY..."
                        className="w-full text-xs py-2 px-3 pr-10 rounded-xl border border-gray-250 dark:border-zinc-800 bg-white dark:bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-zinc-800 dark:text-zinc-100"
                      />
                      <button
                        type="button"
                    ...
                    {customKey && (
                      <button
                        ...
                        className="text-[9px] text-rose-500 hover:underline font-bold mt-1 inline-block"
                      >
                        Quitar clave personalizada (Usar global del servidor)
                      </button>
                    )}
                  </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 md:px-5 space-y-4 bg-gray-50/50 dark:bg-zinc-950/30 flex flex-col">
